...
describe('ifx-overview-table', () => {
  ...
  it('counts planned components correctly', async () => {
    ...
    jest.spyOn(page.root.shadowRoot, 'querySelectorAll').mockImplementation((selector) => {
  ...
  it('counts completed components correctly', async () => {
    ...
    const mockCompletedComponents = Array(40).fill(document.createElement('div'));
    jest.spyOn(page.root.shadowRoot, 'querySelectorAll').mockImplementation((selector) => {
  ...
  it('calculates total components correctly', async () => {
    ...
    jest.spyOn(page.root.shadowRoot, 'getElementById').mockReturnValue(mockIndicator as any);
  ...
  it('updates DOM with component counts', async () => {
    ...
    jest.spyOn(page.root.shadowRoot, 'getElementById').mockImplementation((id) => {
      if (id === 'number__indicator-planned') return mockPlannedIndicator as any;
      if (id === 'number__indicator-completed') return mockCompletedIndicator as any;
  ...
  it('sets header height CSS variable', async () => {
    ...
    jest.spyOn(page.root.shadowRoot, 'querySelector').mockImplementation((selector) => {
    ...
    expect(mockWrapper.style.setProperty).toHaveBeenCalledWith('--header-height', '100px');
