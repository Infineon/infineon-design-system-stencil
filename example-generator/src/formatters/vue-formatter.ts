...
export class VueCodeFormatter implements ICodeFormatter {
  ...
  formatComponent(component: ComponentInfo, options: FormatOptions): string {
    ...
    return this.structureToTemplate(component.structure, component, indent, isFirstOfType);
  }
  ...
  const codeForDisplay = this.generateCodeForDisplay(component, eventHandlers, template);
...
`;
   ...
   */
  private generateCodeForDisplay(component: ComponentInfo, eventHandlers: string, template: string): string {
  ...
  private structureToTemplate(
    ...
    if (struct.children && struct.children.length > 0) {
      const childrenTemplate = struct.children
        .map((child) => this.structureToTemplate(child, componentInfo, indent + '  ', isFirstOfType))
        .join('\n');
