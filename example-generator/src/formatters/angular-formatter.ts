...
import type { ICodeFormatter, FormatOptions } from '../interfaces.js';
import { 
  toPascalCase, 
  toCamelCase, 
  toAngularEventName, 
...
export class AngularCodeFormatter implements ICodeFormatter {
  ...
  formatComponent(component: ComponentInfo, options: FormatOptions): string {
    ...
    return this.structureToHTML(component.structure, component, indent, isFirstOfType);
  }
  ...
  formatEventHandlers(component: ComponentInfo, options: FormatOptions): string {
    ...
    return component.events.map(event => {
      ...
      return `  protected ${handlerName}(event: any) {
  ...
  formatComponentTypeScript(component: ComponentInfo): string {
    ...
    const storyNameSuffix = component.storyName && component.storyName !== 'Default' 
      ? component.storyName.replace(/\s+/g, '') 
      : '';
    const componentClassName = `${componentName}${storyNameSuffix}Example`;
    const selectorSuffix = component.storyName && component.storyName !== 'Default' 
    ...
    const html = this.structureToHTML(component.structure, component, '  ', new Map());
    const tsCode = this.generateTypeScriptCode(component, componentImports, eventHandlers);
...
`;
   ...
   */
  private generateTypeScriptCode(component: ComponentInfo, componentImports: string, eventHandlers: string): string {
    const imports = this.formatImports(component);
    const componentName = toPascalCase(component.component);
    const storyNameSuffix = component.storyName && component.storyName !== 'Default' 
      ? component.storyName.replace(/\s+/g, '') 
...
export class ${componentClassName} {`;
   ...
   * @deprecated Use generateTypeScriptCode instead
   */
  private generateCodeForDisplay(component: ComponentInfo, html: string, eventHandlers: string): string {
    const imports = this.formatImports(component);
    const componentName = toPascalCase(component.component);
...
@Component({
  selector: 'app-${component.component}-example',
  imports: [ ${componentImports} ],
  templateUrl: './${component.component}-example.html',
  styleUrl: './${component.component}-example.scss'
})
export class ${componentName}Example {`;
    ...
    // Template part
    displayCode += `<!-- ${component.component}-example.html -->\n${html}`;
    
    return this.escapeHtml(displayCode);
  }
  ...
  private structureToHTML(
    ...
    const shouldAddEvents = componentInfo.events.length > 0 && 
    ...
    if (shouldAddEvents && isFirst) {
      componentInfo.events.forEach(event => {
        ...
        const handlerName = toHandlerFunctionName(event.name);
        attrs.push(`(${angularEventName})="${handlerName}($event)"`);
      });
    }
    ...
    if (struct.children && struct.children.length > 0) {
      const childrenHTML = struct.children
        .map((child) => this.structureToHTML(child, componentInfo, indent + '  ', isFirstOfType))
        .join('\n');
      return `${openTag}>\n${childrenHTML}\n${indent}</${tag}>`;
    }
   ...
   */
  private toAngularAttribute(name: string, value: string, componentInfo: ComponentInfo): string | null {
    ...
    // Numeric values
    if (!isNaN(Number(attrValue)) && attrValue !== '' && attrValue !== 'null') {
    ...
    if (attrValue.startsWith('[') || attrValue.startsWith('{') || attrValue.includes('[object Object')) {
      const camelName = toCamelCase(attrName);
      const original = (componentInfo?.defaultArgs as any)?.[camelName];
   ...
   */
  private collectComponentTags(struct: ComponentStructure, tags = new Set<string>()): Set<string> {
    tags.add(struct.tag);
    
    if (struct.children && struct.children.length > 0) {
      struct.children.forEach((child) => this.collectComponentTags(child, tags));
    }
