import type { ComponentStructure, ComponentEvent, ComponentInfo, StoryMetadata } from './types.js';
...
export function parseElement(element: Element): ComponentStructure {
  ...
  if (!element.tagName) {
    throw new Error(`Element does not have a tagName property. Received: ${typeof element}, value: ${JSON.stringify(element)}`);
  }
 ...
 */
export function extractEvents(argTypes: Record<string, any>): ComponentEvent[] {
  ...
  Object.entries(argTypes || {}).forEach(([key, value]) => {
    if (value?.action) {
      events.push({
        ...
        patterns: value.table?.type?.detail
      });
    }
  });
...
export async function extractComponentInfo(
  ...
  try {
    ...
    const storiesToExtract: { name: string; export: any }[] = [];
    
    if (storyNames && storyNames.length > 0) {
      // Extract all specified stories that exist
      for (const storyName of storyNames) {
        if (storyModule[storyName]) {
          storiesToExtract.push({ name: storyName, export: storyModule[storyName] });
        }
      }
      if (storiesToExtract.length === 0) {
        throw new Error(`None of the specified stories [${storyNames.join(', ')}] found in ${storyPath}`);
      }
    } else {
      // Fallback: Try multiple story exports: Default, Single, Primary, Basic, Medium, DefaultState
      const fallbackNames = ['Default', 'Single', 'Primary', 'Basic', 'Medium', 'DefaultState'];
      ...
      if (storiesToExtract.length === 0) {
        throw new Error(`No Default/Single/Primary/Basic/Medium/DefaultState export found in ${storyPath}`);
      }
    }
    ...
    for (const story of storiesToExtract) {
      ...
      if (!componentElement) {
        throw new Error(`Story function returned undefined or null. Story path: ${storyPath}, Story: ${usedStoryName}`);
      }
      ...
      if (typeof componentElement === 'string') {
        ...
        componentElement = tempDiv.firstElementChild;
        
        if (!componentElement) {
          throw new Error(`Failed to parse HTML string into DOM element. HTML: ${componentElement}`);
        }
      }
    ...
    throw new Error(`Failed to extract component info from ${storyPath}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
