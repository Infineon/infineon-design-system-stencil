...
import type { IExampleGenerator, GeneratorConfig, GenerationResult } from '../interfaces.js';
import { AngularCodeFormatter } from '../formatters/angular-formatter.js';
import { FileUpdater } from '../utils/file-updater.js';
import { toPascalCase, formatTitle } from '../utils/string-utils.js';
...
export class AngularModuleExampleGenerator implements IExampleGenerator {
  ...
  generate(components: ComponentInfo[], config: GeneratorConfig): GenerationResult {
    ...
    try {
      ...
      for (const component of components) {
        ...
        imports.push(`import { ${componentClassName} } from './generated/${componentFileName}/${componentFileName}';`);
      ...
      const appModulePath = path.join(config.outputDir, 'src', 'app', 'app-module.ts');
      
      if (!this.validate(config)) {
        result.success = false;
        result.errors?.push(`app-module.ts or app.html not found or missing markers: ${appModulePath}`);
        return result;
      }
  ...
  private formatModuleComponentTypeScript(component: ComponentInfo): string {
    ...
    const eventHandlers = this.formatter.formatEventHandlers(component, { indent: '  ' });
...
`;
   ...
   */
  private generateTypeScriptCode(component: ComponentInfo, eventHandlers: string): string {
    const componentName = toPascalCase(component.component);
    const storyNameSuffix = component.storyName && component.storyName !== 'Default' 
      ? component.storyName.replace(/\s+/g, '') 
      : '';
...
export class ${componentClassName} {`;
  ...
  validate(config: GeneratorConfig): boolean {
    ...
    return this.fileUpdater.hasMarkers(appModulePath, [
