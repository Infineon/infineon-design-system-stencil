import { IfxAlert, IfxButton } from '@infineon/infineon-design-system-angular/standalone';
import { Component } from '@angular/core';

@Component({
  selector: 'app-ifx-alert-example',
  imports: [ IfxAlert, IfxButton ],
...
export class IfxAlertExample {
  ...
  protected get htmlCode(): string {
    const controlledAttrs = [
      ...
      ["closable", this.controlledProps["closable"]],
      ["AriaLive", this.controlledProps["ariaLive"]],
    ]
    ...
    __CONTROLLED_ATTRS__&gt;Attention! This is an alert message — check it out!&lt;/ifx-alert&gt;`.replace("__CONTROLLED_ATTRS__", controlledAttrs);
  }
  ...
  protected readonly ariaLiveOptions = ["off","polite","assertive"];
  protected ariaLiveIndex = 2;
  ...
  protected toggleClosable() { this.closable = !this.closable; }
  protected toggleAriaLive() { this.ariaLiveIndex = (this.ariaLiveIndex + 1) % this.ariaLiveOptions.length; }
  ...
  protected get controlledProps(): Record<string, unknown> {
    return {
      ...
      "ariaLive": this.ariaLiveOptions[this.ariaLiveIndex],
    };
  }
