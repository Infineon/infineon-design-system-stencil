...
import { IfxAlert, IfxButton, IfxTextField } from '@infineon/infineon-design-system-react';

export function IfxAlertExample() {
  const [label, setLabel] = useState("Attention! This is an alert message — check it out!");
  ...
  const [ariaLiveIndex, setAriaLiveIndex] = useState(2);
  ...
  const toggleAriaLive = () => setAriaLiveIndex((i) => (i + 1) % ariaLiveOptions.length);
  ...
  const controlledProps = {
    ...
    "closable": closable,
    "ariaLive": ariaLiveOptions[ariaLiveIndex],
  ...
  const formatPropValueForCode = (value: unknown): string => {
    ...
    ["ariaLive", controlledProps["ariaLive"]],
	]
		.map(([name, value]) => `        ${String(name)}=${formatPropValueForCode(value)}`)
		.join("\n");

	const codeStringWithProps = `import { IfxAlert } from '@infineon/infineon-design-system-react';
  ...
  return (
    ...
    <>
      <IfxAlert
        ...
        <IfxButton variant="secondary" onClick={toggleAriaLive}>Toggle AriaLive</IfxButton>
	      </div>
