...
import { IfxButton, IfxContentSwitcher, IfxContentSwitcherItem, IfxIcon, IfxTextField } from '@infineon/infineon-design-system-react';

export function IfxContentSwitcherExample() {
  const [amountOfItems, setAmountOfItems] = useState(4);
  ...
  const formatPropValueForCode = (value: unknown): string => {
    ...
    ["selected", controlledProps["selected"]],
    ["icon", controlledProps["icon"]],
	]
		.map(([name, value]) => `        ${String(name)}=${formatPropValueForCode(value)}`)
		.join("\n");

	const codeStringWithProps = `import { IfxContentSwitcher, IfxContentSwitcherItem, IfxIcon } from '@infineon/infineon-design-system-react';
