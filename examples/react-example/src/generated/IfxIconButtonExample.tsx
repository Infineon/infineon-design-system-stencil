...
import { IfxButton, IfxIconButton, IfxTextField } from '@infineon/infineon-design-system-react';
...
export function IfxIconButtonExample() {
  ...
  const [ariaLabelText, setAriaLabelText] = useState("Icon Button");
  ...
  const handleShapeChange = () => setShapeIndex((i) => (i + 1) % shapeOptions.length);
  ...
  const formatPropValueForCode = (value: unknown): string => {
    ...
    ["shape", controlledProps["shape"]],
    ["ariaLabelText", controlledProps["ariaLabelText"]],
	]
		.map(([name, value]) => `        ${String(name)}=${formatPropValueForCode(value)}`)
		.join("\n");

	const codeStringWithProps = `import { IfxIconButton } from '@infineon/infineon-design-system-react';
  ...
  return (
    ...
    <>
      <IfxIconButton {...(controlledProps as any)} />
        ...
        <IfxButton variant="secondary" onClick={handleShapeChange}>Toggle Shape</IfxButton>
	      </div>
