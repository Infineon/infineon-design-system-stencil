export function render(container) {
  ...
  const stateSpan = container.querySelector('#state');
  const toggleBtn = container.querySelector('#toggle-disabled');

  function update() {
    items.forEach(item => {
      item.disabled = actionListDisabled;
    });

    stateSpan.textContent = String(actionListDisabled);
  }

  toggleBtn.addEventListener('click', () => {
    actionListDisabled = !actionListDisabled;
    update();
  });
