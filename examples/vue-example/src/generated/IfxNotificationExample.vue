...
import { IfxButton, IfxNotification, IfxTextField } from '@infineon/infineon-design-system-vue';

import { computed, ref } from 'vue';

const label = ref("Sample Notification");
const variantOptions = ["success","locked","error","neutral"];
...
const linkHref = ref("https://www.example.com");
...
const toggleLinkText = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; linkText.value = String(raw); };
const toggleLinkHref = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; linkHref.value = String(raw); };
...
const controlledAttrsCode = [
  ...
  ["linkText", controlledProps.value["linkText"]],
  ["linkHref", controlledProps.value["linkHref"]],
  ["linkTarget", controlledProps.value["linkTarget"]],
]
...
<template>
  <div>
    <ifx-notification v-bind="boundProps">
...
<template>
  <div>
	...
	<div class="controls controls-toggle">
      ...
      <IfxButton variant="secondary" @click="toggleLinkTarget">Toggle LinkTarget</IfxButton>
    </div>
	<div class="controls controls-input">
      <IfxTextField label="label" type="text" :value="String(label)" @input="toggleLabel" @ifxInput="toggleLabel" />
      <IfxTextField label="linkText" type="text" :value="String(linkText)" @input="toggleLinkText" @ifxInput="toggleLinkText" />
      <IfxTextField label="linkHref" type="text" :value="String(linkHref)" @input="toggleLinkHref" @ifxInput="toggleLinkHref" />
    </div>
    ...
    <div class="state">
        ...
        <div><b>linkText:</b> {{ String(linkText) }}</div>
        <div><b>linkHref:</b> {{ String(linkHref) }}</div>
        <div><b>linkTarget:</b> {{ String(linkTargetOptions[linkTargetIndex]) }}</div>
    </div>
