...
import { IfxButton, IfxSidebar, IfxSidebarItem, IfxSidebarTitle, IfxTextField } from '@infineon/infineon-design-system-vue';
...
const hrefOfSidebarItem = ref("https://google.com");
const targetOfSidebarItemOptions = ["_self"];
const targetOfSidebarItemIndex = ref(0);
const numberIndicatorOfSidebarItem = ref("");
const activeSidebarItem = ref(false);
const isActionItem = ref(false);
const hideMenuLabel = ref("Hide Menu");
...
const toggleCopyrightText = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; copyrightText.value = String(raw); };
const toggleIcon = () => (iconIndex.value = (iconIndex.value + 1) % iconOptions.length);
const toggleHrefOfSidebarItem = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; hrefOfSidebarItem.value = String(raw); };
...
const toggleNumberIndicatorOfSidebarItem = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; numberIndicatorOfSidebarItem.value = String(raw); };
...
const toggleHideMenuLabel = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; hideMenuLabel.value = String(raw); };
...
<template>
  ...
  position="left"
	...
	<div class="controls controls-toggle">
      <IfxButton variant="secondary" @click="toggleShowHeader">Toggle ShowHeader</IfxButton>
      <IfxButton variant="secondary" @click="toggleShowFooter">Toggle ShowFooter</IfxButton>
      ...
      <IfxButton variant="secondary" @click="toggleIsActionItem">Toggle IsActionItem</IfxButton>
    </div>
	<div class="controls controls-input">
      ...
      <IfxTextField label="imprint" type="text" :value="String(imprint)" @input="toggleImprint" @ifxInput="toggleImprint" />
      <IfxTextField label="termsOfUse" type="text" :value="String(termsOfUse)" @input="toggleTermsOfUse" @ifxInput="toggleTermsOfUse" />
      <IfxTextField label="privacyPolicy" type="text" :value="String(privacyPolicy)" @input="togglePrivacyPolicy" @ifxInput="togglePrivacyPolicy" />
      ...
      <IfxTextField label="hrefOfSidebarItem" type="text" :value="String(hrefOfSidebarItem)" @input="toggleHrefOfSidebarItem" @ifxInput="toggleHrefOfSidebarItem" />
      <IfxTextField label="numberIndicatorOfSidebarItem" type="text" :value="String(numberIndicatorOfSidebarItem)" @input="toggleNumberIndicatorOfSidebarItem" @ifxInput="toggleNumberIndicatorOfSidebarItem" />
      <IfxTextField label="hideMenuLabel" type="text" :value="String(hideMenuLabel)" @input="toggleHideMenuLabel" @ifxInput="toggleHideMenuLabel" />
    </div>
