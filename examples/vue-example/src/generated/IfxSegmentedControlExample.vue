...
const toggleAmountOfSegments = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; amountOfSegments.value = Number(raw); };
const toggleLabelOfSegment = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; labelOfSegment.value = String(raw); };
...
const toggleIfxChange = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; ifxChange.value = String(raw); };
...
const controlledAttrsCode = [
  ["amountOfSegments", controlledProps.value["amountOfSegments"]],
  ["labelOfSegment", controlledProps.value["labelOfSegment"]],
  ...
  ["ifxChange", controlledProps.value["ifxChange"]],
]
...
<template>
  <div>
	...
	<div class="controls controls-input">
      <IfxTextField label="amountOfSegments" type="number" :value="String(amountOfSegments)" @input="toggleAmountOfSegments" @ifxInput="toggleAmountOfSegments" />
      <IfxTextField label="labelOfSegment" type="text" :value="String(labelOfSegment)" @input="toggleLabelOfSegment" @ifxInput="toggleLabelOfSegment" />
      ...
      <IfxTextField label="ifxChange" type="text" :value="String(ifxChange)" @input="toggleIfxChange" @ifxInput="toggleIfxChange" />
    </div>
    ...
    <div class="state">
        ...
        <div><b>ifxChange:</b> {{ String(ifxChange) }}</div>
    </div>
