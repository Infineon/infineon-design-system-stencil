...
const toggleAriaLevelNumber = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; ariaLevelNumber.value = Number(raw); };
const toggleAutoCollapse = () => (autoCollapse.value = !autoCollapse.value);
...
<template>
  <div>
    <ifx-accordion __CONTROLLED_ATTRS__>
      <ifx-accordion-item
        caption="Label"
        :open="true"
        :aria-level-number=3
...
<template>
  ...
  Lorem ipsum dolor sit amet, consectetur adipiscing elit. Praesent volutpat, ligula eu aliquam bibendum, orci nisl cursus ipsum, nec egestas odio sapien eget neque.
	...
	<div class="controls controls-toggle">
      <IfxButton variant="secondary" @click="toggleAutoCollapse">Toggle AutoCollapse</IfxButton>
	...
	<div class="controls controls-input">
      ...
      <IfxTextField label="ariaLevelNumber" type="number" :value="String(ariaLevelNumber)" @input="toggleAriaLevelNumber" @ifxInput="toggleAriaLevelNumber" />
    </div>
