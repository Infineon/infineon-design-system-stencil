...
const toggleAriaLabelOfLink = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; ariaLabelOfLink.value = String(raw); };

const controlledProps = computed(() => ({
  "copyrightText": copyrightText.value,
  "icon": iconOptions[iconIndex.value],
  "href": href.value,
  "ariaLabelOfLink": ariaLabelOfLink.value,
}));
...
const controlledAttrsCode = [
  ...
  ["ariaLabelOfLink", controlledProps.value["ariaLabelOfLink"]],
]
...
<template>
  <div>
    <ifx-footer __CONTROLLED_ATTRS__>
      <div slot="socials">
        <ifx-link
          variant="title"
