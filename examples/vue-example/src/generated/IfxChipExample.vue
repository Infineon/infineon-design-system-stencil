...
const variantOptions = ["single","multi"];
const variantIndex = ref(0);
const themeOptions = ["outlined","filled-light","filled-dark"];
...
const ariaLabel = ref("Chip");
const selected = ref(false);
const value = ref("Item Value");

const toggleAmountOfChipItems = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; amountOfChipItems.value = Number(raw); };
const toggleChipItemLabel = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; chipItemLabel.value = String(raw); };
const togglePlaceholder = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; placeholder.value = String(raw); };
...
const controlledAttrsCode = [
  ["amountOfChipItems", controlledProps.value["amountOfChipItems"]],
  ["chipItemLabel", controlledProps.value["chipItemLabel"]],
  ["placeholder", controlledProps.value["placeholder"]],
  ["size", controlledProps.value["size"]],
  ["variant", controlledProps.value["variant"]],
  ["theme", controlledProps.value["theme"]],
...
<template>
  <div>
    <ifx-chip v-bind="controlledProps">
      <ifx-chip-item
        value="Item Value 1"
        @ifxChange="handleChange">
        Item Label 1
	...
	<div class="controls controls-toggle">
      ...
      <IfxButton variant="secondary" @click="toggleTheme">Toggle Theme</IfxButton>
      ...
      <IfxButton variant="secondary" @click="toggleSelected">Toggle Selected</IfxButton>
    </div>
	<div class="controls controls-input">
      <IfxTextField label="amountOfChipItems" type="number" :value="String(amountOfChipItems)" @input="toggleAmountOfChipItems" @ifxInput="toggleAmountOfChipItems" />
      <IfxTextField label="chipItemLabel" type="text" :value="String(chipItemLabel)" @input="toggleChipItemLabel" @ifxInput="toggleChipItemLabel" />
    ...
    <div class="state">
        ...
        <div><b>selected:</b> {{ String(selected) }}</div>
