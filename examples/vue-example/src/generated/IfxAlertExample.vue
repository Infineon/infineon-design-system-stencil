...
import { IfxAlert, IfxButton, IfxTextField } from '@infineon/infineon-design-system-vue';

import { computed, ref } from 'vue';

const label = ref("Attention! This is an alert message — check it out!");
const variantOptions = ["primary","success","danger","warning"];
...
const ariaLiveOptions = ["off","polite","assertive"];
const ariaLiveIndex = ref(2);
...
const toggleAriaLive = () => (ariaLiveIndex.value = (ariaLiveIndex.value + 1) % ariaLiveOptions.length);
...
const controlledProps = computed(() => ({
  ...
  "ariaLive": ariaLiveOptions[ariaLiveIndex.value],
}));
...
const controlledAttrsCode = [
  ...
  ["closable", controlledProps.value["closable"]],
  ["AriaLive", controlledProps.value["ariaLive"]],
]
...
<template>
  <div>
	...
	<div class="controls controls-toggle">
      ...
      <IfxButton variant="secondary" @click="toggleClosable">Toggle Closable</IfxButton>
      <IfxButton variant="secondary" @click="toggleAriaLive">Toggle AriaLive</IfxButton>
    </div>
    ...
    <div class="state">
        ...
        <div><b>AriaLive:</b> {{ String(ariaLiveOptions[ariaLiveIndex]) }}</div>
    </div>
