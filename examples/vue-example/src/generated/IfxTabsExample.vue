...
import { IfxButton, IfxTab, IfxTabs, IfxTextField } from '@infineon/infineon-design-system-vue';

import { computed, ref } from 'vue';

const amountOfTabs = ref(3);
...
const toggleAmountOfTabs = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; amountOfTabs.value = Number(raw); };
...
const toggleActiveTabIndex = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; activeTabIndex.value = String(raw); };
const toggleHeader = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; header.value = String(raw); };
const toggleDisabled = () => (disabled.value = !disabled.value);

const controlledProps = computed(() => ({
  "amountOfTabs": amountOfTabs.value,
...
<template>
  <div>
	...
	<div class="controls controls-toggle">
      <IfxButton variant="secondary" @click="toggleOrientation">Toggle Orientation</IfxButton>
      ...
      <IfxButton variant="secondary" @click="toggleIconPosition">Toggle IconPosition</IfxButton>
      <IfxButton variant="secondary" @click="toggleDisabled">Toggle Disabled</IfxButton>
    </div>
	<div class="controls controls-input">
      <IfxTextField label="amountOfTabs" type="number" :value="String(amountOfTabs)" @input="toggleAmountOfTabs" @ifxInput="toggleAmountOfTabs" />
      <IfxTextField label="activeTabIndex" type="text" :value="String(activeTabIndex)" @input="toggleActiveTabIndex" @ifxInput="toggleActiveTabIndex" />
      <IfxTextField label="header" type="text" :value="String(header)" @input="toggleHeader" @ifxInput="toggleHeader" />
    </div>

    <div class="state">
        <div><b>amountOfTabs:</b> {{ String(amountOfTabs) }}</div>
        <div><b>orientation:</b> {{ String(orientationOptions[orientationIndex]) }}</div>
