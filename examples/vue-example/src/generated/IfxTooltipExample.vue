...
import { IfxButton, IfxTextField, IfxTooltip } from '@infineon/infineon-design-system-vue';

import { computed, ref } from 'vue';

const text = ref("Hi, I'm a tooltip");
...
const positionOptions = ["auto","bottom-start","top-start","left","bottom-end","top-end","right","bottom","top"];
const positionIndex = ref(0);
const variantOptions = ["compact","dismissible","extended"];
const variantIndex = ref(0);
const header = ref("Tooltip headline");
const ariaLabel = ref("Tooltip with important information");

const toggleText = (event: Event | CustomEvent<{ value?: unknown }>) => { const custom = event as CustomEvent<{ value?: unknown }>; const target = event.target as { value?: unknown } | null; const raw = custom.detail?.value ?? target?.value ?? ''; text.value = String(raw); };
...
const controlledAttrsCode = [
  ["text", controlledProps.value["text"]],
...
<template>
  <div>
	...
	<div class="controls controls-input">
      <IfxTextField label="text" type="text" :value="String(text)" @input="toggleText" @ifxInput="toggleText" />
