...
describe('ifx-action-list-item', () => {
  it('renders with title only (no slots)', async () => {
    ...
    const contentContainer = page.root.shadowRoot.querySelector('.action-list-item__content');
    expect(contentContainer).not.toBeNull();

    const titleElement = contentContainer.querySelector('.action-list-item__title');
    expect(titleElement.textContent).toBe('Test Title');

    // Leading and trailing containers should not be rendered when empty
    const leadingContainer = page.root.shadowRoot.querySelector('.action-list-item__leading');
    const trailingContainer = page.root.shadowRoot.querySelector('.action-list-item__trailing');
  ...
  it('renders with title and description', async () => {
    ...
    const rootDiv = page.root.shadowRoot.querySelector('.action-list-item');
    expect(rootDiv.getAttribute('aria-label')).toBe('Test Title - Test Description');

    const contentContainer = page.root.shadowRoot.querySelector('.action-list-item__content');
    const titleElement = contentContainer.querySelector('.action-list-item__title');
    const descriptionElement = contentContainer.querySelector('.action-list-item__description');
  ...
  it('renders with trailing slot content', async () => {
    ...
    const trailingSlot = trailingContainer.querySelector('slot[name="trailing"]');
  ...
  it('applies clickable state when href is provided', async () => {
    ...
    const rootDiv = page.root.shadowRoot.querySelector('.action-list-item');
    expect(rootDiv.classList.contains('action-list-item--clickable')).toBe(true);
  ...
  it('is not clickable when disabled even with href/value', async () => {
    ...
    const rootDiv = page.root.shadowRoot.querySelector('.action-list-item');
    expect(rootDiv.classList.contains('action-list-item--clickable')).toBe(false);
    expect(rootDiv.getAttribute('tabindex')).toBe('-1');
  });
  ...
  it('emits click event when clicked', async () => {
    ...
    const rootDiv = page.root.shadowRoot.querySelector('.action-list-item') as HTMLElement;
    rootDiv.click();
