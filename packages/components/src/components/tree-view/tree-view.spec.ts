...
describe('ifx-tree-view', () => {
    ...
    it('applies disabled class when disableAllItems is true', async () => {
        ...
        const treeContainer = page.root.shadowRoot.querySelector('.tree-view');
        expect(treeContainer.classList.contains('tree-view--disabled')).toBeTruthy();
    });
    ...
    `,
        ...
        type TreeViewElement = HTMLElement & { expandAllItems: boolean; disableAllItems: boolean };
        type TreeViewItemElement = HTMLElement & { expanded: boolean; hasChildren?: boolean };

        // Verify tree view rendered correctly
        const treeView = page.root as TreeViewElement;
        expect(treeView.shadowRoot.querySelector('.tree-view__label').textContent).toBe('Nested Tree');
        ...
        for (const item of Array.from(allItems)) {
            ...
            if (hasChildItems) {
                (item as unknown as TreeViewItemElement).expanded = true;
            }
        }
        await page.waitForChanges();
        ...
        for (const item of Array.from(allItems)) {
            ...
            if (hasChildItems) {
                ...
                expect((item as unknown as TreeViewItemElement).expanded).toBeTruthy();

                // Then check if the styling is applied correctly
                const treeItemEl = item.shadowRoot.querySelector('.tree-item');
                expect(treeItemEl.classList.contains('tree-item--expanded')).toBeTruthy();
            }
        }
        ...
        const treeItems = Array.from(treeView.querySelectorAll('ifx-tree-view-item'));
        ...
        const subfolder = rootItem.querySelector('ifx-tree-view-item[value="subfolder-1"]') as TreeViewItemElement;
        ...
        expect(subfolder.shadowRoot.querySelector('.tree-item__children')).toBeTruthy();
...
describe('ifx-tree-view-item', () => {
    ...
    it('renders tree item correctly', async () => {
        ...
        const checkbox = page.root.shadowRoot.querySelector('.tree-item__checkbox-container ifx-checkbox');
        expect(checkbox).toBeTruthy();

        // Should have file icon (not folder)
        const fileIcon = page.root.shadowRoot.querySelector('ifx-icon[icon="file-16"]');
    ...
    it('renders tree item with children correctly', async () => {
        ...
        expect(treeItemDiv.classList.contains('tree-item--has-children')).toBeTruthy();

        // Should have chevron
        const chevron = page.root.shadowRoot.querySelector('.tree-item__chevron-container');
        expect(chevron).toBeTruthy();

        // Should have folder icons
        const folderIcon = page.root.shadowRoot.querySelector('ifx-icon[icon="folder-16"]');
    ...
    it('updates checkbox state when clicked', async () => {
        ...
        instance.handleCheckboxChange(new CustomEvent('ifxChange', { detail: { checked: true } }));
        await page.waitForChanges();
