...
export class DatePicker {
  ...
  handleInputFocusOnIconClick() { 
    ...
    // if the browser supports the native API, open the picker
    const anyInput = input as any;
    if (typeof anyInput.showPicker === 'function') {
      anyInput.showPicker();
    } else {
    // fallback – in some browsers click will also open the picker
      input.click();
    }
  }
    ...
    return (
      <div class={`date__picker-container ${this.error ? 'error' : ''} ${this.disabled ? 'disabled': ''}`}>
        ...
        <div class={`input__wrapper ${this.size === 'l' ? 'large' : 'small'} ${this.disabled ? 'disabled' : ''}`}>
          ...
          <div class="icon__wrapper" role="button" tabIndex={0} onClick={() => this.handleInputFocusOnIconClick()}>
