...
|-------------------|---------------------------------|--------|
| `ifxPopoverOpen`  | Emitted when the popover opens  | `{ trigger: HTMLElement \| null }` |
| `ifxPopoverClose` | Emitted when the popover closes | `void` |

```javascript
document.querySelector('ifx-popover').addEventListener('ifxPopoverOpen', (event) => {
