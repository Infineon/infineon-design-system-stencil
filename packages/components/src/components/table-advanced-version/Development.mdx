...
<Controls of={TableStories.DefaultState} />
...
If you update your database and need the table to reflect the changes without losing the current page, use the `refreshCurrentPage()` method:
...
```
This will re-fetch the current page data without resetting the pagination back to page 1.
