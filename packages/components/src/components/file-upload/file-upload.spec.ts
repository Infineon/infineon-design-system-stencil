...
describe('ifx-file-upload', () => {
  ...
  it('should render with default props', async () => {
    ...
    expect(page.root.shadowRoot.querySelector('.file-upload-wrapper')).toBeTruthy();
    expect(page.root.shadowRoot.querySelector('.file-upload-label')).toBeTruthy();
  ...
  it('should be disabled when disabled prop is true', async () => {
    ...
    expect(page.root.shadowRoot.querySelector('.file-upload-wrapper.disabled')).toBeTruthy();
  });
  ...
  it('should display supported file formats info', async () => {
    ...
    const infoText = page.root.shadowRoot.querySelector('.file-upload-info').textContent;
  ...
  it('should reject files with invalid type', async () => {
    ...
    const mockFile = new File(['test content'], 'test-file.exe', { type: 'application/x-msdownload' });
  ...
  it('should reject files exceeding size limit', async () => {
    ...
    const mockFile = new File([mockContent], 'large-file.jpg', { type: 'image/jpeg' });
  ...
  it('should enforce maxFiles limit', async () => {
    ...
    const fileUpload = page.rootInstance;
    const emitExceededSpy = jest.spyOn(fileUpload.ifxFileUploadMaxFilesExceeded, 'emit');
    ...
    const mockFileList2 = {
      ...
      length: 1,
      item: () => mockFile3,
      [Symbol.iterator]: function* () { yield mockFile3; }
    } as unknown as FileList;
...
it('should use custom upload handler when provided', async () => {
  ...
  expect(mockUploadHandler).toHaveBeenCalledWith(mockFile, expect.any(Function));
});
