...
import IconButtonPrimaryVariant from '../../../public-storybook/images/button/icon-button-primary-variant.png';
import IconButtonPrimaryStates from '../../../public-storybook/images/button/icon-button-primary-states.png';
import IconButtonSecondaryVariant from '../../../public-storybook/images/button/icon-button-secondary-variant.png';
import IconButtonSecondaryStates from '../../../public-storybook/images/button/icon-button-secondary-states.png';
import IconButtonSizes from '../../../public-storybook/images/button/icon-button-sizes.png';
import ButtonGroupingImportanceHorizontal from '../../../public-storybook/images/button/button-grouping-by-importance-horizontal.png';
import ButtonGroupingImportanceVertical from '../../../public-storybook/images/button/button-grouping-by-importance-vertical.png';
import ButtonGroupingByFunction from '../../../public-storybook/images/button/button-grouping-by-function.png';
...
Use for actions that are not as important as primary or secondary actions. The tertiary button is a low-contrast button that provides a subtle visual cue to the user.
...
Should be used for actions that result in a destructive action, such as deleting a record. The destructive button has a high-contrast, attention-grabbing style — try to limit its use per page as much as possible.
To make it easier for users to cancel a destructive action, it is recommended to include a secondary button alongside the destructive button on modal components. For instance, if a table is being deleted, users can use the "Cancel" button to close the modal.
...
## Light version
To work on $brand-500 backgrounds there are also light versions of the primary, secondary and tertiary buttons available. The use of the destructive button on $brand-500 backgrounds is not allowed.

## Icon button
This button variant uses only an icon and is the most minimal style. It is available in square and round style. Use it for information dense scenarios with only a limited amount of free interface space like table rows which need a Buy-Action. Or use it as a supportive element for example for accordion or pagination elements.

### Variants

#### Primary
<img src={IconButtonPrimaryVariant} alt="Icon Button Primary Variant" />
Especially the primary square and round variants are the most prominent in their branding appearcance.

##### States
<img src={IconButtonPrimaryStates} alt="Icon Button Primary States" />

#### Secondary
<img src={IconButtonSecondaryVariant} alt="Icon Button Secondary Variant" />
These variant get less attention due to colored border only.

##### States
<img src={IconButtonSecondaryStates} alt="Icon Button Secondary States" />

### Size
The Icon button is available in two different sizes: 32px and 40px
