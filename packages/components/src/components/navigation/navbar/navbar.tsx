...
export class Navbar {
	...
	clearFirstLayerMenu(event: CustomEvent) {
		if (event.detail.action === "hideFirstLayer") {
			...
			for (let i = 0; i < leftMenuItems.length; i++) {
				...
				if (!item.isSameNode(event.detail.component) && this.isNavbarItem(item)) {
					item.hideComponent();
				}
			}
		}
	...
	private handleSidebar(el) {
		...
		const sidebarIconOpen = sidebarIconWrapper.querySelector(
			".navbar__burger-icon",
		);
		const sidebarIconClose = sidebarIconWrapper.querySelector(
			".navbar__cross-icon",
		);
		...
		if (matches) {
			...
			for (let i = 0; i < rightMenuItems.length; i++) {
				...
				if (item.tagName.toUpperCase() === "IFX-NAVBAR-PROFILE") {
					(item as HTMLIfxNavbarProfileElement).showLabel = false;
				} else {
					if (this.isNavbarItem(item) && item.hideOnMobile) {
	...
	render() {
		return (
			<div
				aria-label="a navigation navbar"
				class={`navbar__wrapper ${this.fixed ? "fixed" : ""}`}
			>
				<div class={`navbar__main-container ${this.fixed ? "fixed" : ""}`}>
					...
					>
						...
						>
							<div class="navbar__container-right-content">
								<div class="navbar__container-right-content-navigation-group">
									<div class="navbar__container-right-content-navigation-item-search-bar">
										<div
											class={`navbar__container-right-content-navigation-item-search-bar-icon-wrapper`}
										>
							...
							{/* MOBILE MENU BUTTON */}
							<div
								class={`navbar__burger-icon-wrapper`}
				...
				<div class="navbar__sidebar">
					...
					<div class="navbar__sidebar-bottom-row">
						<slot
							...
							onSlotchange={(e) => this.handleMobileMenuBottom(e)}
						/>
					</div>
				</div>
			</div>
