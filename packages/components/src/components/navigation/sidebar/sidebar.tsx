...
export class Sidebar {
	...
	@Prop() readonly footerHrefTarget: string = "_blank";
	...
	private adjustTopBorder() {
		...
		if (
			children[0].tagName === "IFX-SIDEBAR-ITEM" &&
			children[0].shadowRoot
				.querySelector("div > a")
	...
	private updateCollapsedState() {
		...
		if (this.isCollapsed) {
			...
			this.el.style.setProperty(
				"--ifx-sidebar-item-without-icon-display",
				"none",
			);
			this.el.style.setProperty("--ifx-sidebar-title-display", "none");
			this.el.style.setProperty(
				"--ifx-sidebar-title-show-in-collapsed-display",
				"block",
			);
	...
	private setSidebarItemAttributes(collapsed: boolean) {
		...
		allSidebarItems.forEach((item) => {
			if (collapsed) {
				...
				const isTopLevelItem = item.parentElement === this.el;
				const isSectionHeader =
					item.shadowRoot
						?.querySelector(".sidebar__nav-item")
						?.classList.contains("header__section") === true;
				const isSectionItem = isTopLevelItem && isSectionHeader;

				// In collapsed mode, section headers must be hidden regardless of icon.
				if (!item.icon || isSectionItem) {
					...
					{this.showHeader && (
						<div class="sidebar__nav-bar">
							<div class="sidebar__nav-bar-logo">
								...
								<div class="sidebar__nav-bar-logo-text">
