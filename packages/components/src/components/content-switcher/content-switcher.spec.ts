...
describe('ifx-content-switcher', () => {
  ...
  it('should render with default state', async () => {
    const { root } = await newSpecPage({
      components: [ContentSwitcher, ContentSwitcherItem, MockInfineonIconStencil],
    ...
    expect(root.shadowRoot.querySelector('.ifx-content-switcher')).not.toBeNull();
  ...
  it('should ensure only one item is selected', async () => {
    ...
    expect(root.querySelectorAll('ifx-content-switcher-item[selected]').length).toBe(1);
    expect(root.querySelectorAll('ifx-content-switcher-item')[1].hasAttribute('selected')).toBe(true);
  });
});