...
test("Angular wrapper packages migration assets", {
	...
	try {
		...
		const listing = await listTarball(tarballPath);
...
test("Installed package resolves @angular/compiler and typescript without relying on repo node_modules", {
	...
	try {
		...
		// Install only the wrapper tarball plus its Angular peer — no explicit typescript.
		// typescript must resolve through the wrapper's own runtime dependencies.
		execFileSync("npm", ["install", "--no-save", tarballPath, "@angular/compiler@^20"], {
			cwd: installRoot,
			stdio: "pipe",
			encoding: "utf8",
		});
...
test("prepack: succeeds with current migrations.json and manifest", {
	...
	assert.doesNotThrow(() => {
		runInPackageRoot("node", ["./scripts/prepack.js"]);
	}, "prepack should succeed with valid migrations and manifest");
});
...
test("prepack: fails when a configured migration version is missing from manifest", {
	...
	try {
		// Write a migrations.json that declares a version absent from the manifest
		const fakeCollection = {
			schematics: {
				"update-to-v99": {
					version: "99.0.0",
					description: "Non-existent release",
					factory: "./releases/v99/index.js#updateToV99",
				},
			},
		};
		const collectionPath = path.join(fixtureDir, "migrations.json");
		fs.writeFileSync(collectionPath, JSON.stringify(fakeCollection));

		const { loadReleaseOperations } = require("../migrations/lib/manifest.js");
		const realManifestPath = path.join(PACKAGE_ROOT, "migrations", "shared", "manifest.json");

		assert.throws(
			() => loadReleaseOperations("99.0.0", realManifestPath),
			/99\.0\.0/,
...
test("prepack: fails on invalid manifest schema (malformed operation)", {
	...
	try {
		...
		assert.throws(
			...
			/from.*non-empty/i,
			"Expected error about empty 'from' field",
		);
	} finally {
		await rm(fixtureDir, { recursive: true, force: true });
	}
