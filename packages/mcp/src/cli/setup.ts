...
export async function runSetup(): Promise<void> {
	...
	console.log(
		"This will configure the DDS MCP server in VS Code for this workspace.\n",
	);
	...
	if (!isInWorkspace()) {
		...
		console.error(
			"   Please run this command from your project root (where package.json is located).",
		);
		process.exit(1);
	}
	...
	if (existsSync(mcpConfigPath)) {
		...
		try {
			...
			console.error(
				"⚠️  Warning: Could not parse existing mcp.json. Creating a backup.",
			);
	...
	const localInstalled = existsSync(
		join(process.cwd(), "node_modules", "@infineon", "design-system-mcp"),
	);
	...
	console.log(
		JSON.stringify(
			{ servers: { "design-system": config.servers["design-system"] } },
			null,
			2,
		),
	);
	...
	if (!localInstalled) {
		...
		console.log(
			"   💡 Tip: Install the same version as your DDS package for compatibility",
		);
		console.log("");
	}
	...
	console.log(
		"   2. The DDS MCP server should now be available to your AI assistant",
	);
	console.log(
