...
VanillaJs:.addEventListener("ifxIfxSortChange", (event) => {//handle input});`}}}}},$=e=>{if(e.filterOrientation==="none")return t`<ifx-table
...
</ifx-table>`;{const a=e.columnDefs.map(n=>{const y=[...new Set(e.rowData.map(h=>h[n.field]))].map((h,x)=>t`<ifx-list-entry slot="slot${x}" label="${h}" value="false"></ifx-list-entry>`);return t`
    ...
    `}),v=e.columnDefs.map((n,S)=>{const h=[...new Set(e.rowData.map(i=>i[n.field]))].filter(i=>i!=null&&i!=="").map(i=>({value:i,label:i,selected:!1})),x=JSON.stringify(h);return t`
        <ifx-set-filter slot="filter-component-${S+1}"
            options='${x}'
...
}`,...d.parameters?.docs?.source}}};const B=["DefaultState","Pagination","ServerSidePagination","IncludesButtons","IncludesStatus","IncludesLink","IncludesBreakingline","IncludesInnerButtons","SidebarFilter","TopbarFilter"],_=Object.freeze(Object.defineProperty({__proto__:null,DefaultState:r,IncludesBreakingline:u,IncludesButtons:s,IncludesInnerButtons:m,IncludesLink:c,IncludesStatus:f,Pagination:o,ServerSidePagination:l,SidebarFilter:p,TopbarFilter:d,__namedExportsOrder:B,default:P},Symbol.toStringTag,{value:"Module"}));export{r as D,s as I,o as P,l as S,_ as T,m as a,p as b,d as c};
